...
class Environment:
    ...
    def reset(self):
        ...
        if self.current_episode % 10 == 0.0:    
            ...
            plt.figure(1)
            plt.title('distance error', fontsize=10)
        ...
        if self.new_pose == False:
            ...
            if self.done:
                ...
                with open('/home/andreas/andreas/catkin_ws/src/stalker/scripts/checkpoints/follow'+str(checkpoint)+'/try'+str(ntry)+'/training_error.csv', 'a', newline='') as f:
                    writer = csv.writer(f)
                    data = [ rospy.get_rostime(), self.distance_x/max_distance_x, self.distance_y/max_distance_y, self.x_velocity, self.y_velocity, self.z_position , self.action[0], self.action[1] ]
                    writer.writerow(data)
