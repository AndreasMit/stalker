...
class Environment:
    ...
    def reset(self):
        ...
        with open('/home/andreas/andreas/catkin_ws/src/stalker/scripts/checkpoints/follow'+str(checkpoint)+'/try'+str(ntry)+'/training_reward.csv', 'a', newline='') as f:
            writer = csv.writer(f)
        ...
        if self.current_episode % 10 == 0.0:
            ...
            plt.grid()
            plt.legend()
            plt.savefig('/home/andreas/andreas/catkin_ws/src/stalker/scripts/checkpoints/follow'+str(checkpoint)+'/try'+str(ntry)+'/distancexy-angle'+str(self.ngraph))
            plt.clf()
        ...
        if self.new_pose == False:
            ...
            if self.done:
                ...
                if self.timestep > 1:
                    ...
                    derivative_error = min(abs(self.ddist_x/max_derivative),1) + min(abs(self.ddist_y/max_derivative),1)
                    weight_derivative = 30

                    #velocity error - velocity y we want it to be 0 - velocity x we want to be only positive
                    velocity_error = min(abs(self.y_velocity)/max_velocity, 1) + np.clip((-1)*self.x_velocity/max_velocity, 0, 1)
                    weight_velocity = 70 
                ...
                with open('/home/andreas/andreas/catkin_ws/src/stalker/scripts/checkpoints/follow'+str(checkpoint)+'/try'+str(ntry)+'/training_error.csv', 'a', newline='') as f:
                    writer = csv.writer(f)
