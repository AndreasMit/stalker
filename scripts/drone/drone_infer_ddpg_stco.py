...
class Environment:
    ...
    def __init__(self):
        ...
        self.sub_detector = rospy.Subscriber("/pred_data", PREDdata, self.DetectCallback)
        ...
        if self.new_pose == False:
            ...
            if self.exceeded_bounds and not self.done:
                ...
                with open('src/stalker/scripts/checkpoints/st_co'+str(checkpoint)+'/try'+str(ntry)+'/logfile'+str(nntry)+'.csv', 'a', newline='') as f:
                    writer = csv.writer(f)
                    data = [ rospy.get_rostime(), self.distance/max_distance, self.angle/max_angle, self.x_velocity, self.z_position , self.action[0]/angle_max, self.action[1]/angle_max, self.action[2]/yaw_max ,self.roll/angle_max, self.pitch/angle_max, self.yaw/yaw_max]
                    writer.writerow(data)

                distances.append(self.distance/max_distance)
                angles.append(self.angle/max_angle)
                print(self.distance, self.action[0])
...
if __name__=='__main__':
    ...
    yaw_max = 3.0 #how much yaw should change every time
    yaw_min = -3.0
